...
## chargement base
setwd("/Users/stephaneesquerre/Documents/LA Thèse/Recherche/DFTZ/DATA")
COM_SEC <- read.csv2("WORK_QUERY_FOR_COM_SEC.csv", sep = ";", dec=".", na=".")
...
DFTZ4 <- read.csv2("DFTZ_STATA.csv", sep=";", dec=".", na=".")
DFTZ_TC_VOL <- summaryBy(  PCL ~ as.factor(time) + as.factor(num_TC) , data= DFTZ4, FUN= sum)
names(DFTZ_TC_VOL) <- c("time","num_TC","TC_VOL") #, "num_TC_2119"
DFTZ5 <- merge(DFTZ4, DFTZ_TC_VOL, by=c("time","num_TC"))

# rajout des ZE
setwd("/Users/stephaneesquerre/Documents/LA Thèse/Recherche/Echevinage - impact of court structure/DATA")
ZE <- read.csv2("ZE_CD_POST.csv", sep = ";", dec =",")
DFTZ5 <- merge(DFTZ5, ZE,all.x=TRUE, by=c("I_CD_POST_SIEGE"))
