...
#Code TC et TCABS
numTC_CDPOST <- read.csv2("numTC_CDPOST.csv", sep = ";", dec =".", na="")
numTC_CDPOST$CDCOM_TC <- as.factor(substr(as.character(numTC_CDPOST$CDCOM_TC),2,6)) #encore des code communes sans TC
numTC_CDPOST$CDCOM_TCABS <- as.factor(substr(as.character(numTC_CDPOST$CDCOM_TCABS),2,6)) #encore des code communes sans TC
# on merge les deux
...
names(CD_POST_geoloc) <- c("I_CD_POST_SIEGE","numTC_LAT","numTC_LONG")
TC_CDPOST <- merge(TC_CDPOST, CD_POST_geoloc,all.x=TRUE, by.x=c("CDCOM_TC"), by.y=c("I_CD_POST_SIEGE"))
names(CD_POST_geoloc) <- c("CDCOM_TCABS","numTCABS_LAT","numTCABS_LONG")
TC_CDPOST <- merge(TC_CDPOST, CD_POST_geoloc,all.x=TRUE, by=c("CDCOM_TCABS"))

dt <- TC_CDPOST
library(data.table)
setDT(dt)
dt[, distance_TC := distHaversine(matrix(c(CD_POST_LONG, CD_POST_LAT), ncol = 2), matrix(c(numTC_LONG, numTC_LAT), ncol = 2))]
dt[, distance_TCABS := distHaversine(matrix(c(CD_POST_LONG, CD_POST_LAT), ncol = 2), matrix(c(numTCABS_LONG, numTCABS_LAT), ncol = 2))]
dt <- as.data.frame(dt)
