...
install.packages(c("dplyr","usethis","xlsx","RColorBrewer"))
library(xlsx)
library(knitr)
library(sp)
library(plm)
...
DFTZB$yyyymm <- as.numeric(as.character(DFTZB$ANNEE))*100 + DFTZB$MOIS
...
tc_ville <- merge(tc_ville[,c('num_TC_2009','I_CD_POST_SIEGE')], lien_tc[,c('num_TC_2009','libel_crt','tc_abs','num_TC','TGI','D_TC')], all.x=T, by=c("num_TC_2009"))
...
#merge
#test <- merge(DFTZ4, DFTZ_TC_VOL[,c('num_TC_2009','ANNEEMOIS','TC_VOL')], by=c("ANNEEMOIS","num_TC_2009"))
# rajout des ZE
#PB_ZE <- DFTZ5[is.na(DFTZ5$ZE)==TRUE,]
#write.table(PB_ZE[,c('I_CD_POST_SIEGE','libel_crt')], file = "PB_ZE.csv", row.names = FALSE, sep=";", dec=".", na=".")
ZE_CD_POST <- read.csv2("ZE_CD_POST.csv", sep = ";", dec =",")
ZE_CD_POST$I_CD_POST_SIEGE <- as.factor(substr(as.character(ZE_CD_POST$I_CD_POST_SIEGE),2,6)) #encore des code communes sans TC
ZE_CD_POST$ZE <- as.factor(substr(as.character(ZE_CD_POST$ZE),2,5)) #encore des code communes sans TC
DFTZ5 <- merge(DFTZ4, ZE_CD_POST,all.x=TRUE, by=c("I_CD_POST_SIEGE"))
# rajout des distances
dt <- read.csv2("dist_tc.csv", sep=";", dec=".",na = ".")
DFTZ5 <- merge(DFTZ5, dt,all.x=TRUE, by=c("I_CD_POST_SIEGE"))
# rajout des volumes de pcl avant et après réforme
DFTZ_TC_VOL_2009 <- read.csv2("stat_tc_vol_2009.csv",  sep=";", dec=".",na = ".")
DFTZ_TC_VOL <- read.csv2("stat_tc_vol.csv",  sep=";", dec=".",na = ".")
DFTZ5 <- merge(DFTZ5, DFTZ_TC_VOL_2009[,c('num_TC_2009','ANNEEMOIS','TC_VOL_2009')], by=c("ANNEEMOIS","num_TC_2009"))
DFTZ5 <- merge(DFTZ5, DFTZ_TC_VOL[,c('num_TC','ANNEEMOIS','TC_VOL')], by=c("ANNEEMOIS","num_TC"))
############################
####### Sortie pour stata
write.table(DFTZ5, file = "DFTZ_STATA.csv", row.names = FALSE, sep=";", dec=".", na=".")



#write.table(DFTZ4, file = "DFTZ_STATA_v1.csv", row.names = FALSE, sep=";", dec=".", na=".")
DFTZ_ALL <- merge(DFTZ, tc_ville, by=c("I_CD_POST_SIEGE"))
DFTZ_ALL$PCL <- DFTZ_ALL$RJ + DFTZ_ALL$RJ
#write.table(DFTZ_ALL, file = "DFTZ_STATA_v1.csv", row.names = FALSE, sep=";", dec=".", na=".")
#rajout des stats par date et par TC
#DFTZ_ALL <- read.csv2("DFTZ_STATA_v1.csv", sep=";", dec=".", na=".")
#1. avant réforme
DFTZ_TC_VOL_2009 <- summaryBy(  PCL ~ as.factor(ANNEEMOIS) + as.factor(num_TC_2009) , data= DFTZ_ALL, FUN= sum)
names(DFTZ_TC_VOL_2009) <- c("ANNEEMOIS","num_TC_2009","TC_VOL_2009") #, "num_TC_2119"
write.table(DFTZ_TC_VOL_2009, file = "stat_tc_vol_2009.csv", row.names = FALSE, sep=";", dec=".", na=".")
#1. après réforme
DFTZ_TC_VOL <- summaryBy(  PCL ~ as.factor(ANNEEMOIS) + as.factor(num_TC) , data= DFTZ_ALL, FUN= sum)
names(DFTZ_TC_VOL) <- c("ANNEEMOIS","num_TC","TC_VOL") #, "num_TC_2119"
write.table(DFTZ_TC_VOL, file = "stat_tc_vol.csv", row.names = FALSE, sep=";", dec=".", na=".")
#2.
DFTZ_TCABS_VOL <- summaryBy(  PCL ~ as.factor(time) + as.factor(tc_abs)+ as.factor(num_TC_2009) , data= DFTZ_ALL, FUN= sum)
names(DFTZ_TCABS_VOL) <- c("time","tc_abs","num_TC_2009","TC_VOL") #, "num_TC_2119"
write.table(DFTZ_TCABS_VOL, file = "stat_tcabs_vol.csv", row.names = FALSE, sep=";", dec=".", na=".")
DFTZ_RJ_PCT <- summaryBy(  RJ ~ as.factor(time) + as.factor(tc_abs) + as.factor(num_TC_2009) , data= DFTZ_ALL, FUN= mean)
...
DFTZ_CB_PCT <- summaryBy(  RJ ~ as.factor(time) + as.factor(tc_abs) , data= DFTZ4[DFTZ4$insuff_actif_pct==1,], FUN= mean)
names(DFTZ_CB_PCT) <- c("time","tc_abs","CB_PCT") #, "num_TC_2119"
write.table(DFTZ_CB_PCT, file = "stat_cb_pct.csv", row.names = FALSE, sep=";", dec=".", na=".")



########################################################
########################################################
test <- DFTZ5[!duplicated(DFTZ5[,c('num_TC_2009','tc_abs')]),] #a vérifier
summary(as.factor(test$tc_abs))
...
#Excel simple
DFTZ4 <- DFTZ4[which(DFTZ4$ANNEE>=2006 & DFTZ4$ANNEE < 2013),]
stats_excel <- summaryBy(RJ + LJ  ~ as.factor(time) + as.factor(TC_ABS) , data= DFTZ4, FUN= function(x){c(m = mean(x,na.rm = TRUE))})
write.table(stats_excel, file = "stats_excel.csv", row.names = FALSE, sep=";", dec=",")
